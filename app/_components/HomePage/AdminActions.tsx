...
export default function AdminActions({ onClick }: AdminActionsProps) {
  return (
    <ul className="px-3 flex flex-col gap-3 text-[.9rem]">
      <li className="flex gap-3 items-center hover:text-darkRose1 hover:scale-110 transition-all duration-300">
        <LuCloudUpload className="text-base" />
        <Link href="/upload-cloth" onClick={onClick}>
          Upload Cloth
        </Link>
      </li>
      ...
      <li className="flex gap-3 items-center hover:scale-110 hover:text-darkRose1 transition-all duration-300">
        ...
        <Link href="/suggestions" onClick={onClick}>
          Suggested Wears
        </Link>
      </li>
    </ul>
